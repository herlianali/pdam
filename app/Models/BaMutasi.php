...
class BaMutasi extends Model
{
    ...
    {
        return DB::select("SELECT * FROM (SELECT a.no_bamutasi AS no_ba,a.tgl_bamutasi,b.no_plg,b.zona,b.nama,b.jalan,b.gang,b.nomor,b.notamb, a.kd_tarif_l,a.kd_tarif_b,c.nama_pengadu,e.nama AS NamaPetugas,c.ASAL_PENGADUAN,b.no_bundel FROM ba_mutasi a,dil b,pengaduan c,bonc d,ptgkontrol e WHERE a.no_plg=b.no_plg AND a.no_bonc=d.no_bonc(+) AND a.no_pengaduan=c.no_pengaduan(+) AND d.kd_ptgktrl=e.kd_ptgktrl(+) AND a.no_bamutasi LIKE 'T%' AND a.JNS_MUTASI LIKE '%D%' AND a.FLAG_UBAHTARIF='".$level."' AND to_char(a.TGL_kabag,'yyyymm')='".$periode."' AND a.tgl_batal IS NULL UNION ALL SELECT a.no_bamutkol AS no_ba,a.tgl_bamutasi,c.no_plg,c.zona,c.nama,c.jalan,c.gang,c.nomor,c.notamb, b.kd_tarif_l,b.kd_tarif_b,d.nama_pengadu,f.nama AS NamaPetugas,d.ASAL_PENGADUAN,c.no_bundel FROM ba_mutasikol a,dba_mutasikol b,dil c,pengaduan d,bonc e,ptgkontrol f WHERE a.NO_BAMUTKOL=b.NO_BAMUTKOL AND b.no_plg=c.no_plg AND a.no_bonc=e.no_bonc(+) AND a.no_pengaduan=d.no_pengaduan(+) AND e.kd_ptgktrl=f.kd_ptgktrl(+) AND a.no_bamutkol LIKE 'T%' AND a.JNS_MUTASI LIKE '%D%' AND b.FLAG_UBAHTARIF='".$level."' AND a.BATAL=0 AND a.TGL_BATAL IS NULL AND to_char(a.TGL_kabag,'yyyymm')='".$periode."') a ORDER BY a.no_plg");
    }

    // Mutasi tarif naik turun checkbox terbit 112002
    public static function getTerbit($periode, $level)
    {
        return DB::select("SELECT * FROM (SELECT a.no_bamutasi AS no_ba,a.tgl_bamutasi,b.no_plg,b.zona,b.nama,b.jalan,b.gang,b.nomor,b.notamb,a.kd_tarif_l,a.kd_tarif_b,c.nama_pengadu,e.nama AS NamaPetugas,c.ASAL_PENGADUAN,b.no_bundel FROM ba_mutasi a,dil b,pengaduan c,bonc d,ptgkontrol e WHERE a.no_plg=b.no_plg AND a.no_bonc=d.no_bonc(+) AND d.no_pengaduan=c.no_pengaduan(+) AND d.kd_ptgktrl=e.kd_ptgktrl(+) AND a.JNS_MUTASI LIKE '%D%' AND a.FLAG_UBAHTARIF='".$level."' AND a.BLNTERBIT ='".$periode."' AND a.tgl_batal IS NULL UNION ALL SELECT a.no_bamutkol AS no_ba,a.tgl_bamutasi,c.no_plg,c.zona,c.nama,c.jalan,c.gang,c.nomor,c.notamb,b.kd_tarif_l,b.kd_tarif_b,d.nama_pengadu,f.nama AS NamaPetugas,d.ASAL_PENGADUAN,c.no_bundel FROM ba_mutasikol a,dba_mutasikol b,dil c,pengaduan d,bonc e,ptgkontrol f WHERE a.NO_BAMUTKOL=b.NO_BAMUTKOL AND b.no_plg=c.no_plg AND a.no_bonc=e.no_bonc(+) AND e.no_pengaduan=d.no_pengaduan(+) AND e.kd_ptgktrl=f.kd_ptgktrl(+) AND a.no_bamutkol LIKE 'T%' AND a.JNS_MUTASI LIKE '%D%' AND b.FLAG_UBAHTARIF='".$level."' AND a.BATAL=0 AND a.TGL_BATAL IS NULL AND a.BLNTERBIT='".$periode."' ) a ORDER BY a.no_plg");
    }
